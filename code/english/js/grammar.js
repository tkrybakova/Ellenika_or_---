// ============================================================
// grammar.js – English grammar levels and topics
// ============================================================

const ENGLISH_GRAMMAR_LEVELS = [
  { id: 'a0', name: 'A0', title: 'FOUNDATION', description: 'The absolute basics: words, sentence order and essential forms.' },
  { id: 'a1', name: 'A1', title: 'BEGINNER', description: 'Basic everyday sentences and core grammar.' },
  { id: 'a2', name: 'A2', title: 'ELEMENTARY', description: 'More tenses, comparisons, quantities and everyday structures.' },
  { id: 'b1', name: 'B1', title: 'INTERMEDIATE', description: 'Independent communication and more flexible sentence structures.' },
  { id: 'b2', name: 'B2', title: 'UPPER-INTERMEDIATE', description: 'Complex grammar, nuance and precise expression.' },
  { id: 'c1', name: 'C1', title: 'ADVANCED', description: 'Advanced structures, style and subtle distinctions.' },
  { id: 'c2', name: 'C2', title: 'PROFICIENCY', description: 'Near-native control, precision and sophisticated grammar.' }
];
...
const ENGLISH_GRAMMAR_TOPICS = [
  ...
  { id: 'there_is_are', level: 'A1', name: 'There is / There are', shortDescription: 'Talking about existence and places.' },
  ...
  { id: 'countable_uncountable', level: 'A2', name: 'Countable & uncountable nouns', shortDescription: 'Much, many, some, any and quantities.' },
  { id: 'going_to', level: 'A2', name: 'Going to', shortDescription: 'Plans, intentions and predictions.' },
  { id: 'first_conditional', level: 'A2', name: 'First Conditional', shortDescription: 'Real and possible future conditions.' },
  ...
  { id: 'passive_basic', level: 'B1', name: 'Passive Voice', shortDescription: 'Focusing on the action or result.' },

  { id: 'future_continuous', level: 'B2', name: 'Future Continuous', shortDescription: 'Actions in progress at a future time.' },
  ...
  { id: 'mixed_conditionals', level: 'B2', name: 'Mixed Conditionals', shortDescription: 'Connections between different time periods.' },
  { id: 'modal_perfects', level: 'B2', name: 'Modal Perfects', shortDescription: 'Must have, might have, should have and more.' },
  { id: 'relative_clauses', level: 'B2', name: 'Relative Clauses', shortDescription: 'Defining and non-defining relative clauses.' },
  ...
  { id: 'advanced_modals', level: 'C1', name: 'Advanced Modals', shortDescription: 'Degrees of certainty, criticism and deduction.' },
  { id: 'subjunctive', level: 'C1', name: 'Subjunctive', shortDescription: 'Formal recommendations, demands and hypotheticals.' },
  { id: 'participle_clauses', level: 'C1', name: 'Participle Clauses', shortDescription: 'Compact complex sentence structures.' },
  { id: 'advanced_passive', level: 'C1', name: 'Advanced Passive', shortDescription: 'Complex passive and reporting structures.' },

  { id: 'advanced_conditionals', level: 'C2', name: 'Advanced Conditionals', shortDescription: 'Highly nuanced hypothetical relationships.' },
  ...
  { id: 'discourse_grammar', level: 'C2', name: 'Discourse Grammar', shortDescription: 'Grammar choices across longer texts and contexts.' },
...
function openEnglishGrammar() {
  renderEnglishPage('GRAMMAR', `<div class="english-level-grid">${ENGLISH_GRAMMAR_LEVELS.map(level => `
    <button class="english-level-card" onclick="openEnglishGrammarLevel('${level.id}')">
      <span>${escapeHtml(level.name)}</span><strong>${escapeHtml(level.title)}</strong><small>${escapeHtml(level.description)}</small><b>→</b>
    </button>`).join('')}</div>`);
}

function openEnglishGrammarLevel(levelId) {
  const level = getEnglishGrammarLevel(levelId);
  if (!level) return;
  const topics = ENGLISH_GRAMMAR_TOPICS.filter(topic => topic.level.toLowerCase() === levelId);
  renderEnglishPage(`${level.name} · ${level.title}`, `
    <div class="english-level-header"><span>${escapeHtml(level.name)}</span><p>${escapeHtml(level.description)}</p></div>
    <div class="english-topic-grid">${topics.map(topic => `
      <button class="english-topic-card" onclick="startEnglishGrammar('${topic.id}')">
        <span>${escapeHtml(topic.level)}</span><strong>${escapeHtml(topic.name)}</strong><small>${escapeHtml(topic.shortDescription)}</small>
      </button>`).join('')}</div>`);
}
...
function startEnglishGrammar(id) {
  ...
  renderEnglishPage(topic.name, `
    <div class="english-grammar-detail">
      ...
      ${structure.signals.length ? `<div class="english-signals"><small>COMMON SIGNALS</small><div>${structure.signals.map(signal => `<span>${escapeHtml(signal)}</span>`).join('')}</div></div>` : ''}
