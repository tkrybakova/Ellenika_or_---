...
const PRONOUN_SECTIONS = [
  {
    ...
    table:[['εγώ','μου / εμένα','με / εμένα','—'],['εσύ','σου / εσένα','σε / εσένα','—'],['αυτός','του / αυτού','τον / αυτόν','—'],['αυτή','της / αυτής','την / αυτήν','—'],['αυτό','του / αυτού','το / αυτό','—'],['εμείς','μας / εμάς','μας / εμάς','—'],['εσείς','σας / εσάς','σας / εσάς','—'],['αυτοί','τους / αυτών','τους / αυτούς','—'],['αυτές','τους / αυτών','τις / αυτές','—'],['αυτά','τους / αυτών','τα / αυτά','—']]
  },
  ...
  {id:'reflexive',title:'THE REFLEXIVE PRONOUNS',greek:'Αυτοπαθείς Αντωνυμίες',easy:[['ο εαυτός μου','myself'],['ο εαυτός σου','yourself'],['ο εαυτός του','himself / itself'],['ο εαυτός της','herself']],cases:['Masculine','Feminine','Neuter'],table:[['ο εαυτός μου','η εαυτή μου','το εαυτό μου'],['του εαυτού μου','της εαυτής μου','του εαυτού μου'],['τον εαυτό μου','τον εαυτό μου','το εαυτό μου'],['—','—','—']],note:'The source notes that the Nominative is not often used. Reflexive forms show that the action returns to the person who acted.'}
];
...
function openPronounSection(id){pronounSection=PRONOUN_SECTIONS.find(s=>s.id===id);renderPage('PRONOUNS',renderPronounModes(),'practice-page')}

// Use the exact same F1 level selector as Genders, Declension and Adjectives.
// This intentionally reuses renderLevelButtons so Pronouns cannot drift into a separate button design.
...
function startPronoun(level){pronounLevel=level;pronounQuestion=pronounSection.easy[Math.floor(Math.random()*pronounSection.easy.length)];renderPage('PRONOUNS',renderPronounExercise(),'practice-page')}

function renderPronounExercise(){const s=pronounSection;if(pronounLevel==='easy'){const options=[pronounQuestion[1],...s.easy.filter(x=>x!==pronounQuestion).sort(()=>Math.random()-.5).slice(0,2).map(x=>x[1])].sort(()=>Math.random()-.5);return `<div class="pronoun-exercise f1-practice-card"><div class="exercise-top"><span>${s.title}</span><span>EASY · 01</span></div><div class="progress-track"><i style="width:35%"></i></div><div class="pronoun-prompt">Translate</div><div class="pronoun-word">${pronounQuestion[0]}</div><div class="pronoun-options">${options.map(o=>`<button class="pronoun-option" onclick="checkPronounEasy(this,'${escapePronoun(o)}')">${o}</button>`).join('')}</div><button class="next-action" onclick="startPronoun('easy')">NEXT →</button></div>`}const cols=s.cases;return `<div class="pronoun-exercise f1-practice-card"><div class="exercise-top"><span>${s.title}</span><span>HARD · 02</span></div><div class="progress-track"><i style="width:70%"></i></div><div class="pronoun-prompt">Complete the declension</div><div class="pronoun-word">${pronounQuestion[0]}</div><div class="pronoun-grid">${cols.map((c,i)=>`<label><span>${c}</span><input id="pronoun-cell-${i}" autocomplete="off"></label>`).join('')}</div><button class="check-action" onclick="checkPronounHard()">CHECK</button><div id="pronoun-result" class="pronoun-result"></div><button class="next-action" onclick="startPronoun('hard')">NEXT →</button></div>`}
...
function checkPronounEasy(btn,answer){const correct=normalizePronoun(answer)===normalizePronoun(pronounQuestion[1]);btn.classList.add(correct?'correct':'wrong');if(correct){addStudyPoints(10);showF1Broadcast('Great call — keep the pace!')}else btn.classList.add('answer-reveal')}
function checkPronounHard(){const s=pronounSection;const row=s.table[Math.floor(s.table.length*Math.random())];const cells=s.cases.map((_,i)=>document.getElementById(`pronoun-cell-${i}`));const values=cells.map(x=>normalizePronoun(x?.value));const expected=row.slice(0,cells.length).map(normalizePronoun);const ok=values.every((v,i)=>v===expected[i]||(expected[i]==='—'&&v===''));const result=document.getElementById('pronoun-result');result.textContent=ok?'CORRECT · +20 PTS':`CHECK AGAIN · Example: ${row.join(' · ')}`;result.className='pronoun-result '+(ok?'correct':'wrong');if(ok){addStudyPoints(20);showF1Broadcast('Clean lap. Keep pushing!')}}
